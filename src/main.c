...
gboolean applet_main (MatePanelApplet *applet_widget, const gchar *iid, gpointer data) {
	...
	int stat_res = stat(&applet_home_dir[0], &stat_buf);
        ...
        if ((stat_res == 0) && (!S_ISDIR(stat_buf.st_mode))){
			push_notification(_("Livescore Applet Error"), _("Cannot access configuration directory. Exiting."), NULL);
			return FALSE;
        }
        else if (stat_res == -1) {
                if (errsv == ENOENT) {
                        ...
                        if (mkdir_res == 1) {
				push_notification(_("Livescore Applet Error"), _("Cannot create configuration directory. Exiting."), NULL);
				return FALSE;
			}
                }
                else {
			push_notification(_("Livescore Applet Error"), _("Cannot verify configuration directory. Exiting."), NULL);
			return FALSE;
		}
        }
	...
	if ((stat_res == 0) && (!S_ISREG(stat_buf.st_mode))){
		push_notification(_("Livescore Applet Error"), _("Database file is not a regular file. Exiting."), NULL);
	}
	else if (stat_res == -1) {
		if (errsv == ENOENT) {
			if (!cp(&local_file[0], &skel_file[0])) {
				push_notification(_("Livescore Applet Error"), _("Cannot copy database file to configuration directory. Exiting."), NULL);
				return FALSE;
			}
		}
		else {
			push_notification(_("Livescore Applet Error"), _("Cannot verify database file. Exiting."), NULL);
			return FALSE;
		}
	}
	...
	if (!sqlite_connect(applet)) {
		push_notification(_("Livescore Applet Error"), _("Unable to connect to DB. Exiting."), NULL);
		return FALSE;
	} 
	sqlite3_close(applet->sqlite);
	...
	gtk_action_group_add_actions (applet->action_group, applet_menu_actions, G_N_ELEMENTS (applet_menu_actions), applet);
        ...
        if (res != SQLITE_OK) {
                push_notification(_("Livescore Applet Error"), zErrMsg, NULL);
